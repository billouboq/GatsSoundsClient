...
<div class="videoList" v-infinite-scroll="getMoreVideos" infinite-scroll-disabled="searchLoading" infinite-scroll-distance="800" infinite-scroll-immediate-check="false">
   <md-card class="video" md-with-hover v-for="video of videos">
      <md-card-header>
         <md-avatar>
      ...
      <md-card-actions>
         ...
         <md-button v-bind:class="{active: video.favorite}" class="md-icon-button" @click.native="addRemoveFromFavorite(video)">
...
export default {
   ...
   methods: {
      ...
      addRemoveFromFavorite(video) {
         if (video.favorite) {
            this.$store.dispatch('REMOVE_FROM_FAVORITES', video).then(() => {
               video.favorite = false;
            });
         } else {
            this.$store.dispatch('ADD_TO_FAVORITES', video).then(() => {
               video.favorite = true;
            });
         }
      },
